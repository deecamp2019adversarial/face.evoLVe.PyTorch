...
class HEAD_SOFTMAX_LOSS(nn.Module):
    def __init__(self,head_loss,softmax_loss):
        super(HEAD_SOFTMAX_LOSS, self).__init__()
        self.HEAD = head_loss
        self.LOSS = softmax_loss

    def forward(self,input,labels):
        outputs = self.HEAD(input, labels)
...
if __name__ == '__main__':
    ...
    for epoch in range(NUM_EPOCH): # start training process
        ...
        head_softmax_loss = HEAD_SOFTMAX_LOSS(HEAD,LOSS)
        adversary = pgd_attack(BACKBONE, loss_fn=head_softmax_loss,
        ...
        for inputs, labels in tqdm(iter(train_loader)):
            ...
            random_index = random.sample(range(BATCH_SIZE),int(BATCH_SIZE/2))
            inputs_sampels = inputs[random_index]
            labels_samples = labels[random_index]
            adversary_input = adversary.perturb(inputs_sampels,labels_samples)
            inputs[random_index] = adversary_input
